#!/usr/bin/env python3
"""
Flask server for Tower Defense LLM Trainer - Fixed Static File Serving
Handles OpenAI GPT integration with proper static file serving
"""
...
<body>
    ...
    <div class="status">
        <h2>🚀 System Status</h2>
        <p class="success">✅ Game server is running with REAL LLM integration!</p>
        <p class="success">✅ OpenAI GPT-4o-mini configured and ready</p>
        <p class="error">❌ Static HTML file not found - please check deployment</p>
    </div>
    <div class="status">
        <h2>🔗 API Endpoints</h2>
        <p><strong>/health</strong> - System health check</p>
        <p><strong>/api/llm-guidance</strong> - Get LLM strategic guidance</p>
        <p><strong>/api/elm-predict</strong> - Get ELM predictions</p>
    </div>
</body>
</html>
        """, mimetype='text/html')
...
def health():
    ...
    return jsonify({
        ...
        'model': 'gpt-4o-mini',
        'static_file_exists': os.path.exists(os.path.join(PROJECT_DIR, 'static.html')),
        'timestamp': time.time()
    })
...
if __name__ == '__main__':
    print("🚀 Starting Tower Defense LLM Trainer with FIXED static file serving...")
    print(f"✅ OpenAI API configured: True")
    print(f"🔑 API key length: {len(TEST_OPENAI_API_KEY)}")
    print(f"🤖 Model: gpt-4o-mini")
    print(f"📁 Project directory: {PROJECT_DIR}")
    print(f"📄 Static HTML exists: {os.path.exists(os.path.join(PROJECT_DIR, 'static.html'))}")
